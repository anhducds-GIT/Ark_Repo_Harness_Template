...
| **Tra một bản phát CŨ, hoặc một lộ trình đã xong** | [docs/archive/](docs/archive/CHANGELOG-0.1.0-1.2.20.md) — kho **LƯU TRỮ**: [sổ phát hành 0.1.0→1.2.20](docs/archive/CHANGELOG-0.1.0-1.2.20.md) · [ROADMAP-V1](docs/archive/ROADMAP-V1.md). **Chữ giữ nguyên từng dòng, chỉ đổi chỗ** — file chỉ-thêm phình vô hạn trong một ngân sách cố định. Thư mục này **KHÔNG tính vào ngân sách tài liệu** (`can-nang.mjs` miễn nó): ngân sách đo thứ MỌI phiên phải nạp, mà lưu trữ theo định nghĩa là thứ không nạp mỗi lần. Cất vào đây thì **đừng sửa**. **`HANDOFF.md` KHÔNG cất vào đây được** — cổng đóng phiên đòi nó xoá 0 dòng, xem KHUNG-25 |
