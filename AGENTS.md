...
> **Đây là 16 CỬA VÀO, không phải bản đồ đầy đủ.** Bản đầy đủ — mọi file, kèm *vì sao · có gì
> bên trong · đã vấp ở đâu* — ở [BAN-DO-CHI-TIET](docs/BAN-DO-CHI-TIET.md), và `.repo-structure.json`
> khai nó là bản đồ chính thức. **Thêm file mới thì khai ở ĐÓ.**
>
> Tách 09/09 vì bảng đầy đủ chiếm **10.029 / 13.799 token — 73%** của thứ MỌI phiên phải nạp, ở
> MỌI repo, để rốt cuộc mở một hai file. Nay phần nạp còn **~5.660 token**, và cổng đóng phiên
> canh con số đó (`budget.tokenNap`).

| Khi bạn sắp… | Mở file |
|---|---|
| **Không thấy việc mình sắp làm ở bảng này** | [BAN-DO-CHI-TIET](docs/BAN-DO-CHI-TIET.md) — bản ĐẦY ĐỦ, 43 mục còn lại kèm *vì sao · có gì bên trong · đã vấp ở đâu* |
| Hiểu bộ khung này gồm gì và dùng thế nào | [README](README.md) |
| Ghi một quyết định kiến trúc | [_TEMPLATE-adr](docs/_TEMPLATE-adr.md) · [ADR-0000](docs/adr/0000-ghi-nhan-quyet-dinh-kien-truc.md) |
| **Tra nhanh Đức đã chốt gì, ngày nào** | [decisions](decisions.md) |
| **Có một Ý TƯỞNG, hay muốn biết repo đang có những hướng nào chờ làm** | [IDEAS](IDEAS.md) |
| **Sắp làm cùng lúc với AI khác, hoặc sắp SỬA một trong bốn cơ chế đa phiên** | [MULTIFLOW](docs/protocols/MULTIFLOW.md) |
| Biết phiên trước làm tới đâu | [HANDOFF](HANDOFF.md) · [ADR-0011](docs/adr/0011-cam-rang-cho-ba-tran-kho-chu.md) |
| **Phát sinh việc ngoài phạm vi phiên mình — chỗ ghi nợ, luật mục 0 bắt** | [BACKLOG](BACKLOG.md) · [ADR-0010](docs/adr/0010-xoa-so-quyen-va-tran-so-no.md) |
| **Sắp BÁO CÁO trạng thái cho Đức — kiểm xem điều mình sắp nói có khớp nguồn thẩm quyền không** | `npm run state-check` |
| **Không biết làm gì tiếp, hoặc muốn biết việc nào chạy song song được ngay** | `npm run what-next` |
| **Việc lặp lại — làm theo danh sách kiểm, đừng tự nghĩ lại** | [SO-TAY-AGENT](docs/SO-TAY-AGENT.md) |
| **Tra một thuật ngữ** (gate · claim · lane · fail-closed…) | [LEGEND](docs/LEGEND.md) |
| **Mới vào, chưa biết bắt đầu từ đâu** | [HUONG-DAN](docs/HUONG-DAN.md) |
| **Là phiên ĐIỀU PHỐI: người chốt hỏi "đang có gì · làm gì tiếp · việc nào chạy song song được"** | [ORCHESTRATOR](docs/protocols/ORCHESTRATOR.md) |
| **Xem bảng trạng thái trực quan (trang mẹ)** | [DASHBOARD-Ark-Repo-Har](DASHBOARD-Ark-Repo-Harness.html) · [ADR-0006](docs/adr/0006-bang-mot-khai-niem-mot-cho.md) |
| **Sắp THÊM một luật, hay muốn biết luật nào đang hiệu lực về một chủ đề** | [rule-compiler](scripts/rule-compiler.mjs) · [rule-compiler](tests/rule-compiler.mjs) |
| **Biết bộ khung đang nặng bao nhiêu, và luật nào chưa từng chặn được gì** | `scripts/can-nang.mjs` |
