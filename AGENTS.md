...
| **Một file máy sinh ra liên tục — commit hay để ngoài git?** | [.gitignore](.gitignore) — hiện chỉ giữ ba file bản ra của [bang-song/](bang-song/loi.mjs), và luật là: thứ nào **một tiến trình ghi đè liên tục** thì để ngoài git, vì commit nó là mọi phiên khác mở repo ra thấy cây làm việc bẩn. Đừng lẫn với artifact máy sinh (`DASHBOARD*` · `llms.txt` · `repo-map.json`) — mấy cái đó tất định từ HEAD nên **phải** commit và cổng đối chiếu chúng mỗi phiên. File này còn ghi **một luật đã thử rồi gỡ** (`*.cmd eol=crlf`), giữ lại lý do để không ai thêm lại vì cùng một linh cảm |
