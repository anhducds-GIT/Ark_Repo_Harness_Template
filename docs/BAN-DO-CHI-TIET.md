...
[docs/adr/0012-khoa-muc-file.md](docs/adr/0012-khoa-muc-file.md) — số đo của CHÍNH repo này (620 cặp va chạm, **57% là chặn oan**, p90 **12 file** một lượt sửa), sáu quyết định, và một mục **"cái này KHÔNG chữa"** phải đọc trước khi tin: khoá không giữ file, *git* giữ, và khoá file làm chỗ đó **xấu đi** vì nó bỏ bớt sự serial hoá. Ghim ở [tests/khoa-file.mjs](tests/khoa-file.mjs) — 12 vế, 6 đột biến đã chạy; bốn vế cuối là **dấu niêm phong bảng quyền** (ADR-0017), kéo về từ repo tiêu thụ 09/09. Chỗ *"KHÔNG chữa"* đó đã được vá 10/09: [.githooks/commit-msg](.githooks/commit-msg) là **CỬA INDEX** — `commit-msg` đọc nhãn `Lane:` rồi từ chối lượt commit cuốn theo file lane khác đang khoá (`claim.mjs --cua-index`), vì một cây làm việc có ĐÚNG MỘT index. `--sua` tự bật `core.hooksPath`, cổng ĐỎ nếu tắt. Tầng máy của bản trích nhận nó qua quy tắc **`#!`**, không qua đuôi file — hook không có đuôi, và 1.8.9 đã phát một bản mà sổ phát hành không thấy cửa. Ghim ở [tests/cua-index.mjs](tests/cua-index.mjs) — 12 vế trên **fixture git thật**, 9 đột biến đã chạy; vế 3h tháo cửa ra để chứng minh ca hỏng quay lại, và bốn vế cuối sinh từ một vòng audit độc lập (`--amend` · đường dẫn tiếng Việt · hai bộ đọc nhãn · cửa bị tháo)
