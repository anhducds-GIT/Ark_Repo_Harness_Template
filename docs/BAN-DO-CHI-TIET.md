...
[scripts/luu-do.mjs](scripts/luu-do.mjs) — vẽ khối ```mermaid``` thành **SVG nội tuyến lúc sinh trang**, không nạp thư viện nào. Vì sao không dùng CDN: trang là file tĩnh đem gửi người khác mở, và một lưu đồ chỉ hiện khi có mạng thì đúng lúc cần nhất nó lại là chữ. **Tập con hẹp, cố ý** — `flowchart TD | LR`, `A["nhãn"]`, `A{quyết định}`, `A --> B`, `A -- nhãn --> B`, `-.->`, `-.-`, `---`, chuỗi nhiều nút. Ngoài tập con thì **trả `null`, không ném**, và bên gọi in lại mã nguồn KÈM một dòng nói rõ là chưa vẽ được — lùi im lặng chính là cách lỗi cũ sống 5 tháng. Ghim ở [tests/luu-do-smoke.mjs](tests/luu-do-smoke.mjs), **11 vế** — vế **11** ghim *`md()` LUÔN KẾT THÚC*: nhánh đoạn văn dừng ở mọi dòng mở bằng `|` nhưng nhánh bảng chỉ vào khi dòng sau là hàng ngăn cách, nên một dòng `|` đơn lẻ làm `i` không tăng và vòng quay **vô hạn** — đo 10/09: một tiến trình `build-overview` ở repo đích đốt **65.765 giây CPU (18 tiếng)**. Vế chạy ở **tiến trình con**, vì vòng đồng bộ thì `setTimeout` không bao giờ nổ và vế sẽ **treo cả suite thay vì báo Đỏ**. **8 đột biến đã chạy, 3 cái SỐNG SÓT lượt đầu** vì ba cơ chế giữ nhãn che cho nhau
...
[tests/khoa-dau-vet.mjs](tests/khoa-dau-vet.mjs) — 13 vế; vế **13** ghim *`Audit:` là TRAILER, không phải kiểu commit* — bản cũ quét cả dòng đầu nên một commit kiểu `audit:` bị đọc thành lời khai người duyệt và bị chặn push; ca nặng nhất là tiêu đề `audit:` **cộng** nhãn thật → `AUDIT_XUNG_DOT`, cản đúng commit đã làm đúng. Ghim tín hiệu *"repo chưa thấy dấu vết"*. Vế 7 ghim **mức nghiêm trọng**: tín hiệu này là VÀNG, nó **không** được đổi mã thoát của cổng — chặn một lane đang đọc kỹ là dạy mọi lane ghi bừa một byte để giữ khoá cho hợp lệ. Bốn đột biến đã chạy thật, ghi trong đầu file

### Sắp sửa KHOÁ MỨC FILE, hay thắc mắc vì sao có HAI loại khoá

[docs/adr/0012-khoa-muc-file.md](docs/adr/0012-khoa-muc-file.md) — số đo của CHÍNH repo này (620 cặp va chạm, **57% là chặn oan**, p90 **12 file** một lượt sửa), sáu quyết định, và một mục **"cái này KHÔNG chữa"** phải đọc trước khi tin: khoá không giữ file, *git* giữ, và khoá file làm chỗ đó **xấu đi** vì nó bỏ bớt sự serial hoá. Ghim ở [tests/khoa-file.mjs](tests/khoa-file.mjs) — 12 vế, 6 đột biến đã chạy; bốn vế cuối là **dấu niêm phong bảng quyền** (ADR-0017), kéo về từ repo tiêu thụ 09/09. Chỗ *"KHÔNG chữa"* đó đã được vá 10/09: [.githooks/commit-msg](.githooks/commit-msg) là **CỬA INDEX** — `commit-msg` đọc nhãn `Lane:` rồi từ chối lượt commit cuốn theo file lane khác đang khoá (`claim.mjs --cua-index`), vì một cây làm việc có ĐÚNG MỘT index. `--sua` tự bật `core.hooksPath`, cổng ĐỎ nếu tắt. Tầng máy của bản trích nhận nó qua **ĐƯỜNG DẪN** khai ở `TEP_MAY_THEM` — hook không có đuôi, và 1.8.9 đã phát một bản mà sổ phát hành không thấy cửa; bản 1.8.11 thử quyết bằng `#!` và kiểm toán bác: khi ấy thành viên tầng máy phụ thuộc chính nội dung đang được băm, nên hai hook trái ngược nhau băm ra cùng một dấu. Cùng cửa đó còn canh TẦNG MÁY từ 10/09: mẻ commit chạm tầng máy mà `"version"` không đổi thì bị **chặn tại chỗ (~0,2 giây)** thay vì để suite phát hiện sau **11 phút** — đo được ở chính phiên sinh ra nó. Và [.githooks/post-commit](.githooks/post-commit) **tự trả khoá file** của những file vừa vào commit, giữ nguyên khoá của file chưa commit; mã thoát của nó không ảnh hưởng `git commit` nên nó không thể chặn oan. Ghim ở [tests/cua-index.mjs](tests/cua-index.mjs) — 15 vế trên **fixture git thật**, 15 đột biến đã chạy; vế 3h tháo cửa ra để chứng minh ca hỏng quay lại, bốn vế kế sinh từ một vòng audit độc lập (`--amend` · đường dẫn tiếng Việt · hai bộ đọc nhãn · cửa bị tháo), và vế 8 (11/09) canh chiều NGƯỢC — cửa đó **chặn oan** ai: nó tự tính lại xem một script của repo nhà có chạm được vào bản trích không (nằm trong · đồ thị import · nội dung riêng có lọt vào), nên khai miễn thừa một tên là ĐỎ
