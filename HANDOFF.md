...
> **Lượt CŨ hơn 2026-09-05 đã dời sang** [docs/archive/HANDOFF-2026-09.md](docs/archive/HANDOFF-2026-09.md)
> — chữ giữ nguyên từng dòng, chỉ đổi chỗ để phiên sau không phải nạp 1.272 dòng mỗi lần mở.
...
**Làm gì:** Đức chốt bốn mục đang chờ (KHUNG-11 · 16 · 18 · 23/CP-1). Thi hành cả bốn, phát
bản **1.3.7**.

**Kết quả số:**
- `npm test` → **exit 0**, toàn bộ suite xanh. `tests/core-contract.mjs` **16 xanh, 0 đỏ**
  (thêm F15, F16).
- Đột biến kiểm ba lượt, mỗi lượt phá đúng một chỗ, đòi đúng phép kiểm ấy đỏ, rồi ghi lại byte
  gốc: miễn `claims.json` · nới regex mã việc · vế NÊU TÊN. Cả ba đỏ đúng chỗ.
- Cân nặng: tổng tài liệu **3.681 → 2.999**; `HANDOFF.md` **1.273 → 415** (dưới ngân sách 600
  lần đầu tiên). Hai lượt dời có **md5 đối chiếu với bản trong git** — không mất byte nào.

**BÀI HỌC LỚN NHẤT LƯỢT NÀY — sổ nợ mô tả SAI nguyên nhân, và tôi suýt vá nhầm chỗ.**
KHUNG-16 nói cổng đỏ vì trang nhúng mã commit HEAD. Đo lại thì hai dòng đó **đã được miễn khỏi
phép so từ trước**, nên chúng chỉ làm cây làm việc bẩn. Thứ thật sự làm cổng đỏ là bộ đếm
`CÓ (N commit)`, và nó nhảy vì `.agents/claims.json` mang đuôi `.json` nên bị đếm là file hành
vi — mà nhận/trả quyền là việc MỌI phiên đều làm. Bằng chứng: commit `fa7e8a7` chạm **đúng một
file** là `claims.json`, bộ đếm 4 → 5.

Nếu tin sổ nợ và chỉ làm điều nó bảo, cổng vẫn đỏ y nguyên và tôi đã báo "xong". **Sổ nợ là
giả thuyết của phiên trước, không phải bằng chứng** — luật vàng số 1 áp cho cả sổ của chính mình.

**Bài học thứ hai — công cụ đo tự mâu thuẫn.** `can-nang.mjs` bảo dời nhật ký cũ sang
`docs/archive/`, trong khi nó quét đệ quy cả `docs/`. Làm đúng lời khuyên thì tổng tài liệu
TĂNG. Cùng hình dạng lỗi đã đếm được bốn lần ở repo này: **luật trỏ tới một thứ không hoạt động
như luật tưởng**. Đã vá; `docs/archive/` nay được miễn.

**Còn mở:**
- **KHUNG-11 vòng hai** — còn vượt **799 dòng**. Tôi KHÔNG tự gọt `ORCHESTRATOR.md`: gọt hết
  276 dòng cũng chỉ còn 2.723, **vẫn vượt**, và khối chú thích cuối file bị
  `tests/template-null-repo.mjs` ghim. Cần Đức chọn: gọt thật và chấp nhận mất nội dung, hay
  đặt lại ngân sách theo số đo. **Đây là quyết định của người chốt, không phải của tôi.**
- **KHUNG-23 / CP-1 — cơ chế đã chốt, THI HÀNH thì chưa.** Quyết định "bộ khung thắng" KHÔNG
  phải giấy phép xoá file. Đường đi đã viết thành luật: khai tử luật cũ, giữ văn bản cũ. Mỗi
  lượt thi hành ở từng repo vẫn phải hỏi Đức riêng — riêng `ALL_SKILL_MANAGEMENT` có 1824 dòng
